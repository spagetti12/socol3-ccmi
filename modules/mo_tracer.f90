...
!------------------------------------------------------------------------------
  ...
  USE mo_advection,      ONLY: iadvec,    &! selected advection scheme
                               ...
                               spitfire,       &!
                               tpcore           !
!!$  USE mo_socol_scav,     ONLY: nspec_scav ! eth_as_scav
...
!        trlist% ti(1:nhtrac)% init = RESTART
    ...
    !
    CALL call_init_tracers                                      !MSSOCOL
    ! !!! ECHAM-5 bug !!!                                       !MSSOCOL
    ! IF (PRESENT(xtm1)) THEN                                   !MSSOCOL
    !    CALL call_init_tracers (xt, xtm1)                      !MSSOCOL
    ! ELSE                                                      !MSSOCOL
    !    CALL call_init_tracers (xt)                            !MSSOCOL
    ! ENDIF                                                     !MSSOCOL
    !
...
!==============================================================================
  SUBROUTINE xtdiagn (kproma, kbdim, klev, ktrac, krow, pxtm1, pxtte) !MSSOCOL
  !SUBROUTINE xtdiagn                                                 !MSSOCOL

    ! Scalar arguments
    INTEGER :: kproma, kbdim, klev, ktrac, krow                       !MSSOCOL

    ! Array arguments
    REAL(dp):: pxtm1(kbdim,klev,ktrac), pxtte(kbdim,klev,ktrac)       !MSSOCOL

    CALL call_diagn (kproma, kbdim, klev, ktrac, krow, pxtm1, pxtte)  !MSSOCOL
    !CALL call_diagn                                                  !MSSOCOL

  END SUBROUTINE xtdiagn
!==============================================================================
  SUBROUTINE xtdriver1(kproma, kbdim, klev, klevp1, ktrac, paphp1, papp1,    &
                         ptte, ptm1, pxtm1, pxtte, pqm1, pqte, ktrpwmo, krow,&
                         kglat)  
                                                                      !MSSOCOL
!  SUBROUTINE xtdriver1(kproma, kbdim, klev, klevp1, ktrac, paphp1, papp1,    &
!                        ptte, ptm1, pxtm1, pxtte)                    !MSSOCOL
    ...
    CALL call_chem1 (kproma, kbdim, klev, klevp1, ktrac, paphp1, papp1,    &
                    ptte, ptm1, pxtm1, pxtte, pqm1, pqte, ktrpwmo, krow, kglat)
                                                                      !MSSOCOL
    !CALL call_chem1 (kproma, kbdim, klev, klevp1, ktrac, paphp1, papp1,    &
    !                ptte, ptm1, pxtm1, pxtte)                        !MSSOCOL
...
!                         ptte, ptm1, pxtm1, pxtte)
    ...
    REAL(dp) :: xn3depcv(kbdim,klev,5), cvdprec(kbdim,klev) 
