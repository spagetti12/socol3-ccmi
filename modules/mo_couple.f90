...
!                   paifre  snowfall minus sublimation
...
!----------------------------------------------------------------------
  ...
  SUBROUTINE water_budget_corr
    ...
    !  freshwater correction applied to ice-free ocean points
    ...
    USE mo_memory_g3b,    ONLY: apmebco, rain, awfre, slf, alake, seaice
    ...
    REAL(dp), POINTER  :: zawfre(:,:)  ! freshwater flux [m/s]
    ...
    INTEGER :: jlat
    REAL(dp)    :: zpmeb_glob, zrain_glob, zwater_glob
    REAL(dp)    :: zrain_water, zcorr
...
!  total rain and p-e correction averaged over coupling time step [m/s]
      ...
      DO jlat=1,ngl
        ...
        zrainz(jlat)=SUM(zrain(1:nlon,jlat)*zwater(1:nlon,jlat))*gl_budw(jlat)
        zwaterz(jlat)=SUM(zwater(1:nlon,jlat))*gl_budw(jlat)
      END DO

!  global means of p-e correction, rain over ice-free ocean
!  and ice-free ocean fractional area

      zpmeb_glob=SUM(zpmebz)
      zrain_glob=SUM(zrainz)
      zwater_glob=SUM(zwaterz)

!  rain averaged over ice-free ocean and global freshwater correction
!  applied to ice-free ocean

      zrain_water=zrain_glob/zwater_glob
      zcorr=zpmeb_glob/zwater_glob

!  freshwater correction expressed in terms of a local rain modification over
!  ice-free ocean points with F(corr)=F(uncorr)-rain+rain*(1+zcorr/zrain_water)

      zawfre(:,:) = zawfre(:,:) + zcorr*zrain(:,:)/zrain_water
