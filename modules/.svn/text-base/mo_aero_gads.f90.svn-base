...
!            su_aero_gads,aero_gads,aero_gads4,fcvaer
  PUBLIC :: tauan_ext,pizan_ext,cgan_ext,caern, &
            su_aero_gads,aero_gads,aero_gads4,fcvaer            !MSSOCOL
...
! B)
  ...
  REAL(dp) :: tauan_ext(8,6,12) ! ditto extended to 6 SW bands  !MSSOCOL
  REAL(dp) :: pizan_ext(8,6,12) ! ditto extended to 6 SW bands  !MSSOCOL
  REAL(dp) :: cgan_ext(8,6,12)  ! ditto extended to 6 SW bands  !MSSOCOL
  ! longwave:
...
SUBROUTINE su_aero_gads  ! former suaerx
  ...
  !  Local arrays:
  REAL(dp) :: tauan(8,4,12) !  extinction cross section in cm2/particle !MSSOCOL
  REAL(dp) :: pizan(8,4,12) !  single scattering albedo             !MSSOCOL
  REAL(dp) :: cgan(8,4,12)  !  assymetry factor                     !MSSOCOL
...
!-- 6. Extension to 6 SW bands                                        !MSSOCOL
                                                                      !MSSOCOL
  DO ih = 1, 8                                                        !MSSOCOL
     DO ja = 1, 12                                                    !MSSOCOL
        DO in = 1, 4                                                  !MSSOCOL
           tauan_ext(ih,in+2,ja) = tauan(ih,in,ja)                    !MSSOCOL
           pizan_ext(ih,in+2,ja) = pizan(ih,in,ja)                    !MSSOCOL
           cgan_ext(ih,in+2,ja) = cgan(ih,in,ja)                      !MSSOCOL
        ENDDO                                                         !MSSOCOL
        ! Additional SW bands                                         !MSSOCOL
        tauan_ext(ih,1:2,ja) = tauan(ih,1,ja)                         !MSSOCOL
        pizan_ext(ih,1:2,ja) = pizan(ih,1,ja)                         !MSSOCOL
        cgan_ext(ih,1:2,ja) = cgan(ih,1,ja)                           !MSSOCOL
     ENDDO                                                            !MSSOCOL
  ENDDO                                                               !MSSOCOL
...
!  Marco Giorgetta, MPI, April 2000
  ...
  USE mo_socol_grid_calculations, ONLY: zaetr_socol                   !MSSOCOL
  ...
  ! No tropospheric aerosols in stratosphere:                         !MSSOCOL
  IF (lsocol) THEN                                                    !MSSOCOL
     DO jaer=1,newaer                                                 !MSSOCOL
        aero_gads(:,:,jaer)= (1._dp-zaetr_socol(1:klon,1:klev)) &
             * aero_gads(:,:,jaer)                                    !MSSOCOL
     ENDDO                                                            !MSSOCOL
  ENDIF                                                               !MSSOCOL
...
!
  ...
  IF (lsocol) THEN                                                    !MSSOCOL
     DO jaer=1,newaer                                                 !MSSOCOL
        aero_gads4(:,:,jaer)= (1._dp-zaetr_socol(1:klon,1:klev)) &
             * aero_gads4(:,:,jaer)                                   !MSSOCOL
     ENDDO                                                            !MSSOCOL
  ENDIF                                                               !MSSOCOL
