...
CONTAINS
  ...
  SUBROUTINE WMO_tropopause(kproma, kbdim, klev, &
    ...
    REAL(dp), INTENT(inout) :: pttropo(kbdim)
...
!LK    ENDDO
    ...
    ! Temperature at WMO tropopause

    DO jl = 1, kproma

       jlevst = ktrpwmop1(jl) ! first level below thermal tropopause
       
       zdp = papm1(jl,jlevst)**zkappa-papm1(jl,jlevst-1)**zkappa
       zdpst = papm1(jl,jlevst)**zkappa-ptropo(jl)**zkappa
       
       zdt = ptm1(jl,jlevst) - ptm1(jl,jlevst-1)
       pttropo(jl) = ptm1(jl,jlevst) - zdt*zdpst/zdp
