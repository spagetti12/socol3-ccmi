SUBROUTINE cloud (         kproma, kbdim, ktdia, klev, klevp1, krow    &    ! eth_as_lpj
!!$SUBROUTINE cloud (         kproma, kbdim, ktdia, klev, klevp1          & ! eth_as_lpj
!-----------------------------------------------------------------------
! - INPUT  2D .
                         , paphm1,   pvervel                           &
                         , papm1,    papp1,    pacdnc                  &
...
! eth_as_lpj+
USE mo_lpj_streams,    ONLY : sundur_d, sec_day, tot_precip
USE mo_geoloc,         ONLY : rdayl_x
! eth_as_lpj-
!
  IMPLICIT NONE
!
  INTEGER :: kbdim, klevp1, klev, kproma, ktdia, krow
...
!
  DO 911 jl    = 1,kproma
     ...
     ! eth_as_lpj+
     tot_precip(jl,krow) = tot_precip(jl,krow) + zdtime* &
          (prsfl(jl)+pssfl(jl)) * 86400._dp
     ! eth_as_lpj-
911 END DO
!
...
923 END DO
    DO 924 jl     = 1,kproma
      ...
      ! eth_as_lpj+
      IF (rdayl_x(jl,krow) .EQ. 1._dp) THEN
         sundur_d(jl,krow) = sundur_d(jl,krow) + zdtime * (1._dp -zclcov(jl))
         sec_day(jl,krow)  = sec_day(jl,krow) + zdtime 
      END IF
      ! eth_as_lpj-
924 END DO
!
