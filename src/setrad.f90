SUBROUTINE setrad
  ...
  USE mo_socol_tropoaerosols, ONLY: ndfaer_socol_tropoaer            !MSSOCOL
...
!       co2mmr=co2vmr*amco2/amd
       IF (p_parallel_io) &
            WRITE(nerr,*) 'ico2 =',ico2,' in radctl namelist is not yet supported'
       CALL finish('setrad','Run terminated')
     CASE(2)
       IF (p_parallel_io) &  
     ...
     CASE(10)                                                         !MSSOCOL
       IF (p_parallel_io) THEN                                        !MSSOCOL
          IF (lsocol) THEN                                            !MSSOCOL
             WRITE(nout,*) &                                          !MSSOCOL
                  'iaero= 10 --> Aerosol climatology based on Lohmann et al., 1999'
          ELSE                                                        !MSSOCOL
             WRITE(nerr,*) 'iaero=', iaero, &                         !MSSOCOL
                  ' in radctl namelist is not supported'              !MSSOCOL
             CALL finish('setrad','Run terminated')                   !MSSOCOL 
          ENDIF                                                       !MSSOCOL
       ENDIF                                                          !MSSOCOL
     CASE default
       IF (p_parallel_io) &
     ...
     ! iaero=10: Use tropospheric aerosols based on Lohmann et al., 1999:
     IF (iaero==10) ndfaer=ndfaer_socol_tropoaer                      !MSSOCOL

     IF (iaero==1.OR.iaero==3.OR.iaero==4 .OR. iaero==10) THEN        !MSSOCOL
     !IF (iaero==1.OR.iaero==3.OR.iaero==4) THEN                      !MSSOCOL
     ...
     CASE(4)
        ...
        CALL su_aero_tanre(cetah)
     CASE(10)                                                        !MSSOCOL
        CALL su_aero_gads  ! optical properties of GADS aerosols     !MSSOCOL
     CASE default
       IF (p_parallel_io) &
            WRITE(nerr,*) &
