SUBROUTINE inictl
  ...
  USE mo_control,       ONLY: lamip, lcouple, lipcc, ldebug, lmidatm, lmlo,   &
                              ...
                              lsocol, izcalc_start, izcalc_stop               !MSSOCOL
  ...
  USE mo_advection,     ONLY: iadvec, nmwicfi ! eth_as_20091125
...
!-- 2. Read namelist runctl
  ...
  IF (p_parallel) THEN
     ...
     CALL p_bcast (lsocol, p_io)                                      !MSSOCOL 
     CALL p_bcast (izcalc_start, p_io)
     CALL p_bcast (izcalc_stop, p_io)
