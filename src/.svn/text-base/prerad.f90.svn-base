SUBROUTINE prerad
 ...
 !--  Compute middle of the month for perpetual runs
  ...
  IF (l_orbvsop87) THEN
    ...
    !   - calculate julian day of begin of that year - this is a constant
    !   - add fraction of the current year
    !   - test reference: 1. January 1998 00 UTC === Julian Day 2450814.5
       
      ! julian day of begin of perpetual year:
      yrfix=yr_perp
      mofix=1
      dyfix=1
      jsecfix=0
      CALL Set_JulianDay(yrfix, mofix, dyfix, jsecfix, jul_date_fix)
      zdoyfix = jul_date_fix%day + jul_date_fix%fraction
      
      ! julian day of begin of current year:
      yrfix=yr
      mofix=1
      dyfix=1
      jsecfix=0

      ! correction for current year at first timestep of year: 
      !   - current_date is end of last year, since coupled model begins at last
      !     timestep of previous month
      call get_date_components(next_date,YEAR=yrnext)
      if (yrnext == yr+1) yrfix=yrnext

      ! correction for current year at last timestep of year
      !   - necessary if end of run is at 24:00 of current month (not the case)
      !  call get_date_components(previous_date,YEAR=yrprev)
      !  if (yrprev == yr-1) yrfix=yrprev

      CALL Set_JulianDay(yrfix, mofix, dyfix, jsecfix, jul_date_begin)
      zdoybegin = jul_date_begin%day + jul_date_begin%fraction
      
      ! exchange current year by constant julian day of perpetual year, with
      ! respect to current time in year (zdoy):
      
      CALL Set_JulianDay(yr, mo, dy, jsec, jul_date_now)
      zdoynow = jul_date_now%day + jul_date_now%fraction
      
      zdoy = zdoynow - zdoybegin + zdoyfix
  ...
  IF (l_trigrad) THEN
     ...
     IF (l_orbvsop87) THEN
       ...
       ! fixed orbit at year yr_perp (full radiation time step)
         ...
         ! correction for current year at first and last timestep of year:
         !  - only necessary if full radiation step is at 0:00 (not the case)
         !  call get_date_components(next_date,YEAR=yrnext)
         !  if (yrnext == yr+1) yrfix=yrnext
         ! correction for current year at last timestep of year
         !   - necessary if end of run is at 24:00 of current month (not the case)
         ...
         ! exchange current year by constant julian day of perpetual year, with
         ! respect to current time in year - full radiation step (zdoym):
       
         CALL Set_JulianDay(yr, mo, dy, jsec, jul_date_now)
         zdoynow = jul_date_now%day + jul_date_now%fraction
       
         zdoym = zdoynow - zdoybegin + zdoyfix
     ...
     ! output of test year 1998 once a year
       IF (mo == 1 .AND. dy == 1) THEN  
         ...
         CALL message('prerad',message_text)
         WRITE (message_text,'(a,i6,a,f14.5)') &
                'zdoy of 1 Jan 0:00 of perp year',yr_perp,' =',zdoyfix
         CALL message('prerad',message_text)
         WRITE (message_text,'(a,i6,a,f14.5)') &
                'zdoy of 1 Jan 0:00 of curr year',yr,' =',zdoybegin
         CALL message('prerad',message_text)
       ...
       WRITE (message_text,'(a,f14.5,a,f11.8,a,f13.8)') &
             'zdoym(cor) =', zdoym,' cdissem=',cdissem, &
            ' zdecm(deg)=',zdecmdeg
       CALL message('prerad',message_text)
