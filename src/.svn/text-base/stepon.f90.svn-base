...
SUBROUTINE stepon 
  ...
  USE mo_socol_time_control, ONLY: time_set_socol                      !MSSOCOL
  USE mo_socol_lnox_main,    ONLY: lnox_initialize, lnox_close
  USE mo_socol_namelist,     ONLY: lchem, lch4_wetland, lo3orig, interactivelnox
  USE mo_socol_ch4,          ONLY: read_wetland, read_npp, read_tsurf, &! eth_as_ch4
                                   read_ch4_clim

  USE mo_socol_o3orig,       ONLY: o3orig_read_regions   !O3 transport
  ...
  ! S.K. included for correct amip run
  LOGICAL :: lnewyear, lnewday, lnewmonth
  ...
  IF (interactivelnox) CALL lnox_initialize
  ...
  if (lchem)   CALL read_ch4_clim

  ! eth_as_ch4-

  IF (lo3orig) CALL o3orig_read_regions  ! O3 transport 
...
!$OMP END PARALLEL
     ...
     CALL time_set
     IF (lsocol) CALL time_set_socol                                  !MSSOCOL
...
!-- 2.3   Test for end of run
     ...
     IF (interactivelnox.and.lstop) call lnox_close 
