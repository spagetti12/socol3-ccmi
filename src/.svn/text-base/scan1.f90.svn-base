SUBROUTINE scan1
  ...
  USE mo_socol_namelist,  ONLY: lchem, &                                !MSSOCOL
                                lext_oh ! eth_as_ohclim
  ! eth_as_ohclim+
  USE mo_ohclim,          ONLY: pre_ohclim
  ! eth_as_ohclim-
  ...
  EXTERNAL dyn, ewd, sym2, fftd, ffti, gpc, ltd, maxwind,       &
           ...
           prepare_socol_famcorr_adv, &                               !MSSOCOL
           apply_socol_famcorr_adv                                    !MSSOCOL
...
!call ftrace_region_end('scan1_pre_advect_barrier')
  ...
  IF (ldo_advection) THEN
    ...
    ! Prepare family mass correction:                                 !MSSOCOL
    IF (lsocol) THEN                                                  !MSSOCOL
       IF (lchem) CALL prepare_socol_famcorr_adv                      !MSSOCOL
    END IF                                                            !MSSOCOL
...
!-- 2.8.1 Tendencies of advection and mass correction of dry air
  ...
  IF (ldo_advection) then
    ...
    ! Apply family mass correction:                                   !MSSOCOL
    IF (lsocol) THEN                                                  !MSSOCOL
       IF (lchem) CALL apply_socol_famcorr_adv                        !MSSOCOL
    END IF                                                            !MSSOCOL
