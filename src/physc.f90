...
USE mo_socol_namelist,    ONLY: lchem, lhetchem, lsrb_lya_heating, &
                                lqbonudg, lscav                       !MSSOCOL
USE mo_socol_no_gcm_condens_pscs, ONLY: prepare_no_gcm_condens_pscs, &
                                apply_no_gcm_condens_pscs             !MSSOCOL
USE mo_socol_qbo,         ONLY: qbonudg_socol                         !MSSOCOL
USE mo_socol_sun,         ONLY: srb_lya_heating                       !MSSOCOL
USE mo_socol_scav,        ONLY: nspec_scav, cond_scav
USE mo_lpj_streams,       ONLY: tsurf_cel                             ! eth_as_lpj
...
!
! eth_as_scav+
INTEGER  :: kconbot(ldc%nproma)
REAL(dp) :: xn3depcv(ldc%nproma,nlev,nspec_scav), cvdprec(ldc%nproma,nlev) 
REAL(dp) :: zmratep(ldc%nproma,nlev), zfprec(ldc%nproma,nlev), zfevap(ldc%nproma,nlev) 
REAL(dp) :: zaclc(ldc%nproma,nlev), zmlwc(ldc%nproma,nlev)
REAL(dp) :: totliq(ldc%nproma,nlev)
! eth_as_scav-
! 
!  External subroutines
EXTERNAL geopot, pres, presf, radiation, vdiff, surf, cloud, &
         sicetemp, cucall, radheat, collect, interpolate_socol_bcond  !MSSOCOL
! 
!  Intrinsic functions
INTRINSIC EXP
!
...
! Initialize various variables
  zaclc(1:nproma,:) = 0._dp
  zmratep(1:nproma,:) = 0._dp
  zfprec(1:nproma,:) = 0._dp
  zfevap(1:nproma,:) = 0._dp
  zmlwc(1:nproma,:) = 0._dp
  kconbot(1:nproma) = 0
  xn3depcv(1:nproma,:,:) = 0._dp
  cvdprec(1:nproma,:) = 0._dp
  totliq(1:nproma,:) = 0._dp
...
! 3.14 Interpolate boundary conditions of SOCOL (radiation and chemistry)
!      at chemical / radiation time step:                            !MSSOCOL
   IF (lsocol) CALL interpolate_socol_bcond(krow, nproma, nbdim, nlev, nlevp1, &
          apm1, aphm1, tm1(:,:,krow), tropo(:,krow), itrpwmop1)      !MSSOCOL
...
!   print*, vom(1:nproma,:,krow)
!   print*, vol(1:nproma,:,krow)
!   print*, tte(1:nproma,:,krow)
!   print*, tte(1:nproma,:,krow)
...
!
  ...
  IF (ntrac.GT.0) THEN
     ...
     !                xtm1(:,:,:,krow), xtte(:,:,:,krow))             !MSSOCOL
  
    CALL xtdriver1 (nproma, nbdim, nlev, nlevp1, ntrac, aphp1(:,:),   &
                     app1(:,:), tte(:,:,krow), tm1(:,:,krow),          &
                     xtm1(:,:,:,krow), xtte(:,:,:,krow), qm1(:,:,krow), &
                     qte(:,:,krow), itrpwmo, krow, kglat)                    !MSSOCOL
...
!
 ...
 CALL radheat (nproma, nbdim, nlev, nlevp1,                           &
  ...
  ! Additional heating by Schumann Runge bands and Lyman alpha line: !MSSOCOL
  IF (lsocol .AND. lchem) THEN                                                   !MSSOCOL
     IF (lsrb_lya_heating) CALL srb_lya_heating (krow, nproma, nbdim, &
          nlev, apm1, aphm1, tte(:,:,krow))                                 !MSSOCOL
  ENDIF                                                              !MSSOCOL
!
  IF (ltdiag) THEN
...
!
!
  ! Save qte, xlte and xite before calculation of condensation:       !MSSOCOL
  IF (lsocol) THEN
     IF (lchem .AND. lhetchem .AND. (loconv .OR. locond)) &
          CALL prepare_no_gcm_condens_pscs (nbdim, nproma, nlev, &
          qte(:,:,krow), xlte(:,:,krow), xite(:,:,krow))              !MSSOCOL
  END IF
...
!
     ...
     ! Avoid condensation in PSC region:                              !MSSOCOL
     IF (lsocol) THEN
        IF (lchem .AND. lhetchem) &                                   !MSSOCOL
             CALL apply_no_gcm_condens_pscs (krow, nbdim, nproma, nlev, & 
             app1, qte(:,:,krow), xlte(:,:,krow), xite(:,:,krow))
     END IF
...
!     ------------------------------------------------------------    !MSSOCOL
!                                                                     !MSSOCOL
!*      7.a    QBO nudging:                                           !MSSOCOL
!              --- --------                                           !MSSOCOL
!                                                                     !MSSOCOL
  IF (lsocol) THEN                                                    !MSSOCOL
     IF (lqbonudg) CALL qbonudg_socol(krow, nproma, nbdim, nlev, &
          um1(:,:,krow), vom(:,:,krow), apm1, app1)                   !MSSOCOL
  ENDIF                                                               !MSSOCOL
!
!     ------------------------------------------------------------
!
...
!
  IF(locond) THEN
     ...
     IF (ltimer) CALL timer_start(timer_cloud)
#ifdef ARGCHECK
!!$     CALL cloud( nproma, nbdim, ktdia, nlev, nlevp1             & ! eth_as_lpj
     CALL cloud( nproma, nbdim, ktdia, nlev, nlevp1, krow        &
          , aphm1,             vervel(:,:,krow)                  &
...
!!$     CALL cloud( nproma, nbdim, ktdia, nlev, nlevp1              &  ! eth_as_lpj
     ...
     IF (lscav) CALL cond_scav ( nproma, nbdim, ktdia, nlev, nlevp1 &                       
                , ilab, aclcacm(:,:,krow), aphm1, aphp1 &          
                , apm1, app1, geom1, qm1(:,:,krow), tm1(:,:,krow) &        
                , xlm1(:,:,krow), xim1(:,:,krow), xtec(:,:,krow) &           
                , loland, itype &                               
                , aclcovm(:,krow), xlvim(:,krow), xivim(:,krow), aprlm(:,krow), qvim(:,krow) &     
                , zaclc, aclcac(:,:,krow) &                                    
                , aclcov(:,krow), xlvi(:,krow), xivi(:,krow), aprl(:,krow), qvi(:,krow), ssfl &  
                , tte(:,:,krow), qte(:,:,krow), xlte(:,:,krow), xite(:,:,krow) &             
                , aprs(:,krow), rsfl &                                  
...
!DIR$ CONCURRENT
    DO jl=1,nproma
      ...
      ! eth_as_lpj+
      tsurf_cel(jl,krow)=tsurf_cel(jl,krow)+zdtime*(ztsurf - 273.15_dp)
      ! eth_as_lpj-
    END DO
...
!
  ...
  IF (ntrac>0) THEN
    ...
    CALL xtdriver2 (nproma, nbdim, nlev, nlevp1, ntrac, aphp1(:,:),    &
     app1(:,:), tte(:,:,krow), tm1(:,:,krow), xtm1(:,:,:,krow), &
     xtte(:,:,:,krow), krow, kglat, &                            !MSSOCOL
     ztvm1(:,:), &        ! eth_as_scav
...
!     app1(:,:), tte(:,:,krow), tm1(:,:,krow), xtm1(:,:,:,krow), xtte(:,:,:,krow))
    ...
    CALL xtdiagn (nproma, nbdim, nlev, ntrac, krow, xtm1(:,:,:,krow), &
         xtte(:,:,:,krow))                                            !MSSOCOL
    !CALL xtdiagn                                                     !MSSOCOL
  END IF
!
