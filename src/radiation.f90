...
!!$                     pco2,                           &
  ...
  USE mo_socol_ghg_ods , ONLY: co2_bcond_rad, ch4_bcond_rad,       &
                               n2o_bcond_rad, cfc_bcond_rad,       &
                               ch4_3d_bcond_rad, n2o_3d_bcond_rad, &
                               cfc_3d_bcond_rad, ch4_chemrad,      &
                               n2o_chemrad, cfc_chemrad,           &
                               o3_chemrad, ghg_message               !MSSOCOL
  USE mo_socol_tropoaerosols, ONLY: socol_tropoaero                  !MSSOCOL
...
!!$        zco2(1:kproma,:)=MAX(pco2(1:kproma,:),EPSILON(1._dp))
     ...
     CASE (11)    ! no coupling, globally constant                    !MSSOCOL
        zco2(1:kproma,:)=co2_bcond_rad                                !MSSOCOL
     CASE default
     ...
     CASE (10)    ! coupling with CTM                                 !MSSOCOL
        zch4(1:kproma,:)=MAX(ch4_chemrad(jrow,kproma,klev), EPSILON(1._dp))
     CASE (11)    ! no coupling, globally constant                    !MSSOCOL
        zch4(1:kproma,:)=ch4_bcond_rad                                !MSSOCOL
     CASE (12)    ! no coupling, 3d                                   !MSSOCOL
        zch4(1:kproma,:)=MAX(ch4_3d_bcond_rad(:,:), EPSILON(1._dp))   !MSSOCOL
     CASE default
     ...
     CASE (4)
        ...
        pao3(1:kproma,:)=zo3(1:kproma,:)
     CASE (10)   ! coupling with CTM                                  !MSSOCOL
        zo3(1:kproma,:) =MAX(o3_chemrad(jrow,kproma,klev), EPSILON(1._dp))
        pao3(1:kproma,:)=zo3(1:kproma,:)                              !MSSOCOL
     CASE default
     ...
     CASE (10)    ! coupling with CTM                                 !MSSOCOL
        zn2o(1:kproma,:)=MAX(n2o_chemrad(jrow,kproma,klev), EPSILON(1._dp))
     CASE (11)    ! no coupling, globally constant                    !MSSOCOL
        zn2o(1:kproma,:)=n2o_bcond_rad                                !MSSOCOL
     CASE (12)    ! no coupling, 3d                                   !MSSOCOL
        zn2o(1:kproma,:)=MAX(n2o_3d_bcond_rad(:,:), EPSILON(1._dp))   !MSSOCOL
     CASE default
     ...
     CASE (4)
        ...
        END DO
     CASE (10)    ! coupling with CTM                                 !MSSOCOL
        zcfcs(1:kproma,:,:)=MAX(cfc_chemrad(jrow,kproma,klev), EPSILON(1._dp))
     CASE (11)    ! no coupling, globally constant                    !MSSOCOL
        DO jl=1,kproma
           DO jk=1,klev
              zcfcs(jl,jk,:)=MAX(cfc_bcond_rad(:), EPSILON(1._dp))    !MSSOCOL
           ENDDO
        ENDDO
     CASE (12)    ! no coupling, 3d                                   !MSSOCOL
        zcfcs(1:kproma,:,:)=MAX(cfc_3d_bcond_rad(:,:,:), EPSILON(1._dp))
     CASE default
        CALL finish('radiation','cfcs: this "icfc" is not supported')
     END SELECT cfc

     ! Print GHGs at lower boundary:                                  !MSSOCOL
     IF (jrow .EQ. 1) CALL ghg_message(zco2(1,klev),zch4(1,klev),&
          zn2o(1,klev),zcfcs(1,klev,:))                               !MSSOCOL  
     ...
     CASE (4)
        ...
        zsaer(1:kproma,:,4)=0._dp
     CASE (10)                                                        !MSSOCOL 
        zsaer(1:kproma,:,1:naer)=0._dp           !No Tanre aerosols   !MSSOCOL
        zsaer(1:kproma,:,naer+1:naer+newaer)= &
             socol_tropoaero(kproma,kbdim,klev,zdp)                   !MSSOCOL
     CASE default
