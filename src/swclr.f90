...
USE MO_AERO_GADS  , ONLY : RTAUAN=>TAUAN_ext, RPIZAN=>PIZAN_ext, &
                           RCGAN=>CGAN_ext                            !MSSOCOL
USE MO_RADIATION  , ONLY : NDFAER, &
                           iaero                                      !MSSOCOL
USE MO_SW         , ONLY : NOVLP ,NSW
USE mo_control    , ONLY : lsocol                                     !MSSOCOL
USE mo_socol_strataerosols, ONLY : tau_strataer_sw, &                 !MSSOCOL
                           omega_strataer_sw, g_strataer_sw           !MSSOCOL
...
DO JK = 1 , KLEV
  ...
  DO JAE=KAER+1,KAER+KNEWAER
    ...
    DO JL = KIDIA,KFDIA
      IF (iaero .EQ. 10 .AND. (jae-kaer .EQ. 3 .OR. jae-kaer .EQ. 4 .OR. &
           jae-kaer .EQ. 7 .OR. jae-kaer .EQ. 9)) THEN                !MSSOCOL
         ! Relative humidity class 1 in case of hydrophobic           !MSSOCOL
         ! SOCOL aerosols:                                            !MSSOCOL
         IH=1                                                         !MSSOCOL
      ELSE                                                            !MSSOCOL
         IH=KAERH(JL,IKL)
      ENDIF                                                           !MSSOCOL
  ...
  ! Contribution of stratospheric aerosols:                           !MSSOCOL
  ! (NB: Stratospheric aerosols are stored from top to bottom!)       !MSSOCOL
  IF (lsocol) THEN                                                    !MSSOCOL
     DO jl = kidia, kfdia                                             !MSSOCOL
        PTAUAZ(jl,jk) = PTAUAZ(jl,jk) + tau_strataer_sw(jl,ikl,knu)   !MSSOCOL
        PPIZAZ(jl,jk) = PPIZAZ(jl,jk) + tau_strataer_sw(jl,ikl,knu)* &!MSSOCOL
             omega_strataer_sw(jl,ikl,knu)                            !MSSOCOL
        PCGAZ(jl,jk)  = PCGAZ(jl,jk)  + tau_strataer_sw(jl,ikl,knu)* &!MSSOCOL
             omega_strataer_sw(jl,ikl,knu)*g_strataer_sw(jl,ikl,knu)  !MSSOCOL
     ENDDO                                                            !MSSOCOL
  ENDIF                                                               !MSSOCOL
