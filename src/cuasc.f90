...
210 END DO
  DO 230 jk=1,klev
     DO 220 jl=1,kproma
        plu(jl,jk)=0._dp
        pmfu(jl,jk)=0._dp
        zpmfun(jl,jk)=0._dp ! eth_as_20090731
        pmfus(jl,jk)=0._dp
        pmfuq(jl,jk)=0._dp
        pmful(jl,jk)=0._dp
        plude(jl,jk)=0._dp
...
! eth_as_20090731-
  DO 3112 jt=1,ktrac
     DO 3110 jl=1,kproma
        ...
        ENDIF
!       pmfuxt(jl,klev,jt)=pmfub(jl)*pxtu(jl,klev,jt)       ! eth_as_20090731
        pmfuxt(jl,klev,jt)=zpmfun(jl,klev)*pxtu(jl,klev,jt) ! eth_as_20090731
3110 END DO
3112 END DO
!
  DO 320 jl=1,kproma
       ...
       IF(loflag(jl)) THEN
         IF (pqu(jl,jk).LT.zqold(jl)) THEN
           ...
           IF(zbuo.GT.0._dp.AND.pmfu(jl,jk).GE.0.01_dp*pmfub(jl).AND.  &
             ...
             plu(jl,jk)=zlnew
           ELSE
             zpmfun(jl,jk)=0. ! eth_as_20090731
             klab(jl,jk)=0
             pmfu(jl,jk)=0._dp
           END IF
         END IF
...
455  END DO
     DO 4554 jt=1,ktrac
        DO 4552 jl=1,kproma
           IF(loflag(jl)) THEN
!             pmfuxt(jl,jk,jt)=pxtu(jl,jk,jt)*pmfu(jl,jk)   ! eth_as_20090731
              pmfuxt(jl,jk,jt)=pxtu(jl,jk,jt)*zpmfun(jl,jk) ! eth_as_20090731
           ENDIF
4552    END DO
4554 END DO
!
...
!DIR$ IVDEP
  DO 530 jl=1,kproma
     IF(ldcum(jl)) THEN
        ...
        END IF
        zpmfun(jl,jk)=pmfu(jl,jk) ! eth_as_20090731
     END IF
530 END DO
  DO 5312 jt=1,ktrac
     DO 5310 jl=1,kproma
        IF(ldcum(jl)) THEN
           jk=kctop(jl)-1
!          pmfuxt(jl,jk,jt)=pxtu(jl,jk,jt)*pmfu(jl,jk)   ! eth_as_20090731
           pmfuxt(jl,jk,jt)=zpmfun(jl,jk)*pxtu(jl,jk,jt) ! eth_as_20090731
        ENDIF
5310 END DO
5312 END DO
!
